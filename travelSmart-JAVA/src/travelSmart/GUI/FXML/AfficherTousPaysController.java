...
public class AfficherTousPaysController implements Initializable, InterfaceScreenController {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        listPays = paysDAO.displayAllPays();
        for (int i = 0; i < listPays.size(); i++) {
            int idPays = listPays.get(i).getIdPays();
            
            if ((idPays % 2) == 0) {
                panes1.add(loadOnePays(listPays.get(i).getIdPays()));
                
                pays_list1.setItems(panes1);
                pays_list1.setSelectionModel(null);
            }
            
            else {
                panes.add(loadOnePays(listPays.get(i).getIdPays()));
                
                pays_list.setItems(panes);
                pays_list.setSelectionModel(null);
            }
        }
    ...
    private Pane loadOnePays(int idPays) {

        PaysDAO pDAO = new PaysDAO();
        Pays pays = new Pays();

        pays = pDAO.findPaysById(idPays);
        ...
        Label nom_pays = new Label(pays.getNomPays());
        Label capital = new Label("Capitale: "+pays.getCapital());
        ...
        nom_pays.setStyle(styleNom);
        nom_pays.setLayoutX(88);
        nom_pays.setLayoutY(5);
        nom_pays.setId("nom_pays");
        
        capital.setLayoutX(93);
        capital.setLayoutY(30);
    ...
    public void showHome(ActionEvent e) {
        ...
        stackEcrans.unloadScreen("listePays");
    }

 ...
 * @author bechir
 */
public class AffichetoutCompagniesController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        List<Compagnies> listeComp = new ArrayList<Compagnies>();
        listeComp = compDAO.DisplayAllCompagnies();
        for (int i = 0; i < listeComp.size(); i++) {
            int idcomp = listeComp.get(i).getIdCompagnie();

            if ((idcomp % 2) == 0) {
                panes1.add(loadOnePays(listeComp.get(i).getIdCompagnie(), listeComp.get(i).getNom_comp()));

                ville_list1.setItems(panes1);
                ville_list1.setSelectionModel(null);
            } else {
                panes.add(loadOnePays(listeComp.get(i).getIdCompagnie(), listeComp.get(i).getNom_comp() ));
    ...
    private Pane loadOnePays(int idcomp, String nomPays) {

        CompagniesDAO compDAO = new CompagniesDAO();
        Compagnies Compagnies = new Compagnies();

        Compagnies = compDAO.findCompagniesById(idcomp);
        ...
        String styleNom = "-fx-padding: 8 15 15 15;\n"
                + "    -fx-background-insets: 0,0 0 5 0, 0 0 6 0, 0 0 7 0;\n"
                + "    -fx-background-radius: 8;\n"
                + "    -fx-font-weight: bold;\n"
                + "    -fx-font-size: 18;";

        Label nomComp = new Label(Compagnies.getNom_comp());
        Label typeComp = new Label(Compagnies.getType());
        

        // Label Proprieties
        //nom_pays.setPrefSize(314, 61);
        typeComp.setId("typeComp");
        nomComp.setLayoutX(93);
        //nom_ville.setLayoutY(7);
        nomComp.setId("nomComp");

        typeComp.setLayoutX(93);
        typeComp.setLayoutY(30);
        typeComp.setId("nomComp");
